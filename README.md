...
START → research → evaluate → route
        route: overall≥75 or retry≥MAX → extract → END
               부족                      → refine → research (루프백)
```

- **루브릭 평가**: 단일 점수가 아니라 `정량성·방향성·커버리지`(각 0~100) + `overall` + `missing`(부족 항목)으로 채점. 통과 기준 `overall ≥ 75`. 외부 리서치는 '시나리오 레버의 방향성 근거'를 잡는 단계라 직급별 정밀 통계(사내 HR 지표)는 기대하기 어려우므로, 방향성·규모감 중심으로 채점한다.
...
- **상수**(`research_agent.py` 상단): `MAX_RETRY=3`, `EVAL_PASS_THRESHOLD=75`, 평가/보강은 Sonnet, 추출 Haiku, 검색 Sonnet(web search — 검색·요약엔 Opus급 추론이 불필요해 비용·지연 절감).
- **화면**: 라운드별 3축 루브릭 막대 + 종합점수 추이(🏆=추출 사용 라운드) + refine 보강쿼리 테이블 + mermaid 그래프 다이어그램이 실시간 로그와 함께 표시. (통과 기준선·막대 색은 `EVAL_PASS_THRESHOLD`를 단일 출처로 따라간다.)
- **mock 폴백**: 키가 없어도 `58→68→86`으로 점수가 오르며 2회 보강 후 통과하는 흐름을 재현(데모 중단 방지).
...
- **고도화 로드맵**:
  - 데이터 규모 충분 시 **계층적 풀링/베이지안(디리클레)** 추정으로 전이확률 안정화 가능.
  - **유입(inflow) 항 추가**: 현재 투영은 재직 stock의 전이만 다뤄 **신규채용 유입이 없어 총원이 단조 감소**한다(차장 누수 스토리를 또렷이 보이게 하려는 의도). 실모델에선 연도별 채용 계획을 `n(t+1) = n(t)·P + 채용벡터(t)` 형태의 유입 항으로 더하면 stock 균형까지 표현 가능. 본 데모는 **"현 인력 stock의 자연 감쇠 기준선이며, 채용은 갭에 대응하는 별도 레버"**로 프레이밍한다.
