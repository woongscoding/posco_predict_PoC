...
# ★ 임계값 75: 외부 리서치는 '시나리오 레버의 방향성 근거'를 잡는 단계라
#   공개 정량통계(직급별 이직률 등 — 원래 사내 HR 지표)는 기대하기 어렵다.
#   80은 도달 불가능한 바였음 → 방향성+신뢰출처 확보 수준(75)으로 현실화.
EVAL_PASS_THRESHOLD = 75   # overall 이 점수 이상이면 '충분'
...
# =============================================================
# ★ mock 시퀀스: 58 → 68 → 86 으로 점수가 오르며 통과(임계값 75 기준 2회 부족→통과).
...
MOCK_SEARCH_ROUNDS = [
    ...
    # 2회차: 정량 보강됐으나 커버리지 일부 부족 → 통과 직전
    {
        ...
        "rubric": {
            "정량성": 66, "방향성": 76, "커버리지": 62, "overall": 68,
            "missing": ["주니어(사원) 이탈 트렌드 근거"],
            "feedback": "이직률·리텐션 방향성은 확보됐으나 주니어 이탈 트렌드 커버리지가 약함.",
        },
...
def _real_research(keyword: str, queries: Optional[list], round_no: int) -> list[str]:
    ...
    if queries:
        ...
        instruction = (
            ...
            f"각 트렌드의 '방향성(증가/감소)'과 '대략적 규모감'을 명확히 적고, "
            f"수치·증감폭이 있으면 함께 포함해줘(없어도 방향성 위주로 정리).\n쿼리:\n{target}"
        )
    else:
        instruction = (
            f"다음 키워드의 최신 인력시장 동향을 web search로 조사해 핵심 사실을 "
            f"불릿 5개로 요약해줘. 각 동향의 '방향성(인력 유입/유출, 증가/감소)'을 분명히 적고, "
            f"가능하면 증감폭·비율 같은 규모감을 함께 포함해줘.\n"
            f"키워드: {keyword}"
        )
...
def _real_evaluate(keyword: str, snippets: list[str]) -> dict:
    ...
    resp = client.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": (
                "너는 HR 전략 애널리스트다. 아래 검색결과가 '인력 전이확률 조정계수(=시나리오 레버)'의 "
                "근거로 쓸 만한지 다음 3축 루브릭으로 0~100점 채점하라.\n"
                "★ 중요: 이 단계의 목표는 외부 동향의 '방향성과 대략적 규모감'을 잡는 것이다. "
                "직급별·연차별 정밀 이직률 같은 공개 통계는 보통 존재하지 않으며, 그건 원래 "
                "사내 HR 데이터의 몫이다. 따라서 그런 정밀 통계의 부재를 이유로 점수를 깎지 마라.\n"
                "- 정량성: 추세를 뒷받침하는 '수치·비율·증감폭'이 있는가(반정량/근사치·'증가/감소 폭' 언급도 인정). "
                "방향만 있고 규모감이 전혀 없을 때만 낮게 줘라\n"
                "- 방향성: 전이확률을 어느 방향(↑/↓)으로 조정할지 명확한가 (가장 중요한 축)\n"
                "- 커버리지: 핵심 트렌드 3종(노동시장 과열·구조조정·처우개선)을 포괄하는가\n"
                "기준 앵커: 75점 = 트렌드 3종 대부분의 방향성이 신뢰할 출처로 확인 + 일부 규모감. "
                "85+ = 3종 모두 방향성 명확 + 다수 트렌드에 수치/증감폭 근거.\n"
...
def run_research_agent(keyword: str,
    ...
    # ---------- MOCK 폴백 ----------
    # 점수가 오르며(58→68→86) 통과하고, refine 보강 쿼리도 화면에 노출되게 재현.
    search_results = []
