...
def _real_refine(keyword: str, snippets: list[str], missing: list[str],
    ...
    """평가의 missing 항목 + 지금까지 검색 내용을 보고, 새롭고 구체적인
    한국어 검색 쿼리 2~3개를 생성한다. → 점수 정체(같은 쿼리 반복) 해소."""
    client = _anthropic_client()
    seen = "\n".join(snippets[-8:])   # 직전 결과 일부만 (중복 회피용 컨텍스트)
    ...
    resp = client.messages.create(
        ...
        messages=[{
            ...
            "content": (
                ...
                "이미 검색한 내용과 겹치지 않게, 부족 항목을 메우는 '새롭고 구체적인' "
                "한국어 검색 쿼리 2~3개를 만들어라. 연도·직급·지표(이직률/잔류율 등)를 "
                "포함해 구체적으로. JSON 배열만 출력: [\"쿼리1\",\"쿼리2\"]\n\n"
                f"원 키워드: {keyword}\n이미 검색한 내용(일부):\n{seen}"
            ),
        }],
    )
...
def build_graph(emit: EmitFn = _noop):
    ...
    def research_node(state: ResearchState) -> ResearchState:
        ...
        # next_queries 소비 후 초기화
        return {"search_results": prev + snippets, "next_queries": []}
    ...
    def evaluate_node(state: ResearchState) -> ResearchState:
        ...
        emit({"type": "evaluate", "round": state.get("retry_count", 0) + 1,
              ...
              "message": (f"📊 평가 {score}점 (정량 {rubric['정량성']}·"
                          f"방향 {rubric['방향성']}·커버 {rubric['커버리지']}) — {rubric['feedback']}")})
        return {"eval_score": score, "eval_feedback": rubric["feedback"],
...
def run_research_agent(keyword: str,
    ...
    Returns dict:
        {"mode", "trends", "coefficients", "history":[루브릭 평가이력], "search_results"}
