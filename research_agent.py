...
# ★ 임계값 72: 외부 리서치는 '시나리오 레버의 방향성 근거'를 잡는 단계라
#   공개 정량통계(직급별 이직률 등 — 원래 사내 HR 지표)는 기대하기 어렵다.
#   80은 도달 불가능한 바였음 → 방향성+신뢰출처 확보 수준(72)으로 현실화.
EVAL_PASS_THRESHOLD = 72   # overall 이 점수 이상이면 '충분'
...
# =============================================================
# ★ mock 시퀀스: 58 → 68 → 86 으로 점수가 오르며 통과(임계값 72 기준 2회 부족→통과).
    ...
    resp = client.messages.create(
        ...
        messages=[{
            ...
            "content": (
                ...
                "기준 앵커: 72점 = 트렌드 3종 대부분의 방향성이 신뢰할 출처로 확인 + 일부 규모감. "
